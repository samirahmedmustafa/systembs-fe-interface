...
export class NeighbourhoodComponent {
  displayedColumns: string[] = ['id', 'name', "locality", 'edit', 'delete'];
  dataSource: MatTableDataSource<Neighbourhood>;

  @ViewChild(MatPaginator)
  paginator!: MatPaginator;
  @ViewChild(MatSort)
  sort!: MatSort;
  resource: string = "/api/neighbourhoods";
  ...
  edit(item: any): void {
    ...
    this.service.get(this.resource, item.id).subscribe(
      (data: Neighbourhood) => {
        ...
        const dialogRef = this.dialog.open(NeighbourhoodFormComponent, {
          width: '600px',
          height: '400px',
